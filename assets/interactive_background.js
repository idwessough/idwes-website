...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Checkbox element
    const backgroundToggleCheckbox = document.getElementById('backgroundToggle');
    backgroundToggleCheckbox.checked = true; // Enable the interactive background by default
    ...
    function animate() {
        if (backgroundToggleCheckbox.checked) {
            ...
            ctx.clearRect(0, 0, canvas.width, canvas.height); // Clear canvas when disabled
        }
    }
    ...
    backgroundToggleCheckbox.addEventListener('change', function() {
        if (this.checked) {
            init();
            animate();
        } else {
